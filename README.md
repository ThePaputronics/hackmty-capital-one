# F.R.E.D — Open SPEI Detection Engine

F.R.E.D is designed as API as a Service: a worker continuously analyzes SPEI
transactions, detects suspicious activity associated with CLABEs, and updates
a persistent risk database. Institutions consume already-processed signals
through the API; an API read or customer transfer does not trigger the analysis.
Banco de México is the proposed central operator, with open APIs and rules,
protected data, and financing still undefined.

The Sentinel prototype in this checkout supplies a background simulation
worker, synchronous evaluation, and event/evaluation persistence. The continuous
risk materializer and its serving contract are product architecture requirements,
not capabilities established by the existing evaluation endpoint. National-scale
operation and institutional adoption remain proposals. The backend package and
API names remain `sentinel` / Sentinel.

See [the web presentation](site/index.html), [product positioning](docs/business/fred-open-spei-positioning.md),
and [sources and current validation](site/sources.md). The technical prototype
is described below.
...
3. **Stateless API, AWS-Scalable:**
   All state lives in standard PostgreSQL with Alembic migrations. Containerized; evaluation latency is measured by the local benchmark and does not establish national-scale capacity.
4. **Zero-Logic UI:**
...
Latency: p50 = 2.19 ms | p95 = 3.64 ms (Local harness, Python 3.14 / SQLite)
------------------------------------------------------------------
...
This synthetic run was repeated on 2026-09-12 with seed 42 and 30 active days.
Both decision strategies achieved the same reported rates in this sample; it
does not establish comparative superiority or production performance. Full
results are recorded in [site/benchmark.json](site/benchmark.json).
